...
<h6 class="cent-refresh"><a class="gold-bt" onclick="loadview('edit_period/<?php echo $per['id'];?>')" ><i class="entypo-arrows-ccw"></i> Page Refresh</a></h6>
<?php //print_r($per);?>
<hr>
<!--=====================================-->
<div class="guardian">
    <div class="col-sm-6">
        ...
        <div class="panel panel-success" data-collapsed="0">
            ...
            <div class="panel-body">
                ...
                <?php echo form_open_multipart('admin/update_period',$data) ?>
                <br>
                    ...
                    <div class="col-xs-8">
                        <select class="form-control" name="name"  required >
                            <?php foreach($period as $row){?>
                                <option value="<?php echo $row['id'];?>" <?php if($row['id']==$per['name']){ echo 'selected';} ?>><?php echo $row['name'];?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-8">
                        <select type="text" class="form-control" name="class_id" id="class_id" value="" onChange="getSection(value);">
                            ...
                            <?php foreach($class as $row){ ?>
                                ...
                                <option value="<?php echo $row['class_id'];?>" <?php if($row['class_id']==$per['class_id']){ echo 'selected';} ?>><?php echo $row['name'];?></option>
                            <?php }?>
                        </select>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select id="section" class="form-control" name="section_id">
                            <option value="<?php echo $per['section_id']?>"><?php  $section=$this->admin_model->section_by_id($per['section_id']); echo $section['name']?></option>
                        </select>
                    </div>
                </div>
                <!--=====================================-->
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="day" id="day"  value="<?php echo $per['day'];?>">
                    </div>
                </div>
                <div class="form-group">
                    <label for="field-1" class="col-sm-4 control-label">Start Time:</label>
                    <div class="col-sm-8">
                        <input type="time" class="form-control" name="start_time" id="start_time" value="<?php echo $per['start_time'];?>">
                    </div>
                </div>
                <div class="form-group">
                    <label for="field-1" class="col-sm-4 control-label">End Time :</label>
                    <div class="col-sm-8">
                        <input type="time" class="form-control" name="end_time" id="end_time" onblur="chk_time()"  value="<?php echo $per['end_time'];?>">
                    </div>
                </div>
                <!--=====================================-->
                    ...
                    <div class="col-xs-8">
                        <select class="form-control" name="teacher_id" onchange="detail(value)" required >
                            ...
                            <?php foreach($emp_teacher as $row){?>
                                <option value="<?php $x=$this->admin_model->teacher_by_employee_id($row['employee_id']); echo $x['teacher_id'];?>"  <?php if($x['teacher_id']==$per['teacher_id']){ echo 'selected';} ?>><?php echo $row['name'];?></option>
                            <?php } ?>
                        </select>
                    ...
                    <div class="col-xs-8">
                        <select class="form-control" name="subject"  required >
                            <?php foreach($subject as $row){?>
                                <option value="<?php echo $row['id'];?>" <?php if($row['id']==$per['subject']){ echo 'selected';} ?>><?php echo $row['name'];?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-xs-8">
                        <select class="form-control" name="opt_sub"  required >
                            <?php foreach($subject_option as $row){?>
                                <option value="<?php echo $row['id'];?>" <?php if($row['id']==$per['opt_sub']){ echo 'selected';} ?>><?php echo $row['name'];?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function(e){
        ...
        $("#fupForm").on('submit', function(e){
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url()?>index.php/admin/update_period',
