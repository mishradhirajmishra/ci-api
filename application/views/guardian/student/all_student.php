...
<table id="example">
    ...
    <?php foreach($students as $row){?>
        <tr >
            ...
            <td><img style="width:80px !important;"  class="guardian_img zoom" src="<?php echo base_url()?>/uploads/<?php echo $row['student_image'] ?>"> </td>
            <td><?php echo $row['student_name'] ?></td>
            <td><?php $class= $this->guardian_model->class_by_id($row['class']); echo $class['name'] ?></td>
            <td><?php $section= $this->guardian_model->section_by_id($row['section']); echo $section['name']; ?></td>
            <td><?php $guardian= $this->guardian_model->list_guardian_by_id($row['guardian']); echo $guardian['guardian_name']; ?></td>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <table class="table table-responsive">
                                    ...
                                    <tr><th>Class</th><td><?php $class= $this->guardian_model->class_by_id($row['class']); echo $class['name'] ?></tr>
                                    <tr><th>Section</th><td><?php $section= $this->guardian_model->section_by_id($row['section']); echo $section['name']; ?></tr>
                                    <tr><th>Nationality</th><td><?php echo $row['nationality'] ?></tr>
                                    <tr><th>Religion</th><td><?php echo $row['Religion'] ?></tr>
                                    <tr><th>Guardian</th><td><?php $guardian= $this->guardian_model->list_guardian_by_id($row['guardian']); echo $guardian['guardian_name']; ?></tr>
                                    ...
                                    <tr><th>Co-Scholastic Activities (hobbies)</th><td><?php $x= json_decode($row['coscholastic_activities']); $length=count($x);
                                      for($i=0; $i<$length;$i++){
                                          echo ($i+1)." :- ".$x[$i]."&nbsp;&nbsp;&nbsp;&nbsp;";
                                      }
...
<script>
    ...
    function change_status(id,status) {
        ...
        $.ajax({
            url: '<?php echo base_url()?>index.php/teacher/change_student_status',
            type:"POST",
            datatype:"json",
