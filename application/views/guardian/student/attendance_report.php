...
<h6 class="cent-refresh"><a class="gold-bt" onclick="loadview('attendance_report')">Page Refresh</a></h6>
<hr>
<!--=====================================-->
    ...
    <div style="display: none">
        ...
        <div class="col-sm-2 col-xs-8" style="margin-bottom: 10px;display: none">
            <select id="section" class="form-control" >
                <option value="<?php echo $sec;?>"><?php $x=$this->guardian_model->section_by_id($sec); echo $x['name'];?></option>
            </select>
        </div>
   </div>
...
<div class="col-xs-12">
    <div class="edate " style="text-align: center" >
         <span style="color: red">class</span> : <?php $xy=$this->guardian_model->class_by_id($cl); echo $xy['name'];?>
        ...
        <div class="table-wrap">
            <table class="main-table">
                ...
                <tr>
                    ...
                    <th class="fixed-side fixed" >Attendance
                    </th>
                  <?php
                  for($d=1; $d<=$day; $d++){
                      $att=$this->guardian_model->get_attendance_individual($cl,$sec,$row['student_id'],$d,$month,$year);
                      echo "<td>";
                      if($att['attendance']==1){echo "P";}
                      elseif($att['attendance']==2){echo "L";}
                      elseif($att['attendance']==3){echo "A";}
                      echo "</td>";
                  } ?>
                </tr>
                    <tr>
                        <th class="fixed" style="width: 20px;"><?php echo $s++; ?> <i class="entypo-right-dir"></i></th>
                        <th class="fixed-side fixed" >Assessment
                        </th>
                        <?php
                        for($d=1; $d<=$day; $d++){
                            $att=$this->guardian_model->get_assessment_individual($cl,$sec,$row['student_id'],$d,$month,$year);
                            echo "<td >";
...
<script>
    function getdata(month) {
      ...
      var cl = $("#class").val();
       var sec = $("#section").val();
       loadview('attendance_report/'+stu+'/'+cl+'/'+sec+'/'+month);
    }
...
<script>
    ...
    function getSection(value){
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '<?php echo base_url()?>index.php/guardian/section_by_class_id/'+value,
